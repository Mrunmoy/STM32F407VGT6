...
/* App-defined error codes passed as cpromise_set_value's/cpromise_drop's
 * error_code, and read back out of cfuture_wait_for's outError on success.
 * 0 (kStorageErrorOk) is the only non-error value, by libcfuture's own
 * convention. Kept well clear of libcfuture's own CFUTURE_ERR_* range
 * (-1..-6) so a caller can always tell which layer an error came from. */
 ...
 * cpromise_set_value(&request.promise, &result, errorCode). Same struct
 ...
 *                                success/failure is conveyed only via
 *                                cfuture_wait_for's outError.
